...
var configSchema = exports_external.object({
  ...
  evms: exports_external.array(exports_external.object({
    vaultManagerAddress: exports_external.string(),
    consumerAddress: exports_external.string(),
    mockAaveAddress: exports_external.string(),
...
var fetchSupplyRate = (runtime2, evmConfig) => {
...
var fetchAllYieldRates = (runtime2) => {
  const results = [];
  for (const evmConfig of runtime2.config.evms) {
    try {
      runtime2.log(`Fetching supply rate from ${evmConfig.chainSelectorName}...`);
      const supplyRate = fetchSupplyRate(runtime2, evmConfig);
      runtime2.log(`${evmConfig.chainSelectorName} supplyRate: ${supplyRate.toString()} (${Number(supplyRate) / 10000000000000000}%)`);
      results.push({
        chainSelectorName: evmConfig.chainSelectorName,
      ...
      runtime2.log(`Failed to fetch yield from ${evmConfig.chainSelectorName}: ${err}`);
    }
  }
  return results;
};
var updateYieldDataOnChain = (runtime2, evmConfig, allYieldInfos) => {
  ...
  for (const yieldInfo of allYieldInfos) {
    try {
      runtime2.log(`Updating ${evmConfig.chainSelectorName} with chain ${yieldInfo.chainSelectorName} rate: ${Number(yieldInfo.supplyRate) / 10000000000000000}%`);
      const rawReport = encodeAbiParameters(parseAbiParameters("uint64 chainSelector, uint256 supplyRate"), [yieldInfo.chainSelector, yieldInfo.supplyRate]);
      ...
      runtime2.log(`Updated ${evmConfig.chainSelectorName} with ${yieldInfo.chainSelectorName} rate. txHash: ${bytesToHex(resp.txHash || new Uint8Array(32))}`);
    } catch (err) {
      runtime2.log(`Failed to update ${yieldInfo.chainSelectorName} on ${evmConfig.chainSelectorName}: ${err}`);
    }
  }
};
var detectAnomaly = (runtime2, current) => {
  const ANOMALY_THRESHOLD = 500000000000000000;
  for (const info of current) {
    if (info.supplyRate > BigInt(ANOMALY_THRESHOLD)) {
      runtime2.log(`ANOMALY DETECTED on ${info.chainSelectorName}: supplyRate ${Number(info.supplyRate) / 10000000000000000}% exceeds threshold`);
      return true;
    }
  }
  return false;
};
var pauseAllVaults = (runtime2) => {
  for (const evmConfig of runtime2.config.evms) {
    try {
      const network248 = getNetwork({
        chainFamily: "evm",
      ...
      runtime2.log(`Emergency pause ${evmConfig.chainSelectorName}: ${resp.txStatus} txHash: ${bytesToHex(resp.txHash || new Uint8Array(32))}`);
    } catch (err) {
      runtime2.log(`Failed to pause vault on ${evmConfig.chainSelectorName}: ${err}`);
    }
  }
};
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log("=== CrossYield CRE Cycle Start ===");
  runtime2.log(`Scheduled at: ${payload.scheduledExecutionTime}`);
  runtime2.log(`Monitoring ${runtime2.config.evms.length} chains`);
  runtime2.log("--- Step 1: Fetching yield rates from all chains ---");
  const allYieldInfos = fetchAllYieldRates(runtime2);
  if (allYieldInfos.length === 0) {
    throw new Error("Failed to fetch yield data from all chains");
  }
  runtime2.log(`Yield data fetched from ${allYieldInfos.length} chains:`);
  for (const info of allYieldInfos) {
    runtime2.log(`  ${info.chainSelectorName}: ${Number(info.supplyRate) / 10000000000000000}%`);
  }
  runtime2.log("--- Step 2: Anomaly detection ---");
  const isAnomaly = detectAnomaly(runtime2, allYieldInfos);
  if (isAnomaly) {
    runtime2.log("Anomaly detected! Pausing all vaults...");
    pauseAllVaults(runtime2);
    return JSON.stringify({ status: "paused", reason: "anomaly_detected" });
  }
  runtime2.log("No anomaly detected, proceeding...");
  runtime2.log("--- Step 3: Updating yield data on all VaultManagers ---");
  for (const evmConfig of runtime2.config.evms) {
  ...
  const summary = {
    ...
    yieldRates: allYieldInfos.map((info) => ({
      ...
      apy: `${Number(info.supplyRate) / 10000000000000000}%`
    }))
  };
  runtime2.log("=== CrossYield CRE Cycle Complete ===");
  runtime2.log(safeJsonStringify(summary));
